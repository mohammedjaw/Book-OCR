...
from ..package_service import import_package, import_collection_package
...
@router.post('/transfer/import-collection')
def import_collection(request:Request,file:UploadFile=File(...)):
    try: uid=import_collection_package(file.file); return RedirectResponse(f'/collections/{uid}',303)
    except ValueError as e: return templates.TemplateResponse(request=request,name='transfer.html',context={'error':str(e)},status_code=400)
