...
def make_json(uid):
    ...
    f=tempfile.NamedTemporaryFile(suffix='.json',delete=False); Path(f.name).write_text(json.dumps(data,ensure_ascii=False,indent=2),encoding='utf-8'); return f.name,filename(book['title'],'.json')
...
def make_docx(uid):
    ...
    for i,p in enumerate(pages):
        ...
        text=p['extracted_text'] if p['status']=='completed' and p['extracted_text'] else '[فشل استخراج هذه الصفحة]'
        if p['status']=='failed' and p['error_message']: text += '\n'+p['error_message']
        para=doc.add_paragraph(); para.paragraph_format.alignment=2; para.add_run(text)
    f=tempfile.NamedTemporaryFile(suffix='.docx',delete=False); doc.save(f.name); return f.name,filename(book['title'],'.docx')
